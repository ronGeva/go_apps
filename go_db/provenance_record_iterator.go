...
// First, it's easy to see why this iterator finishes in a finite time (does not iterate indefinitely) - we only
// return records that exists in the joint table (each candidate is created from offsets in indexes in the joint
// table).
// In addition, we never return the same record twice thanks to the "visited" map.
// Since there is a finite amount of records in the joint table, and we return each of them no more than once,
// we always stop after finite amount of steps.
//
// Next, we'll prove every record in the joint record is eventually retrieved.
// Every record in the joint table can be represented via a len(table) tuple of offsets (each points to an offset
// in the corresponding table index).
// Those offsets are obviously all greater or equal to 0, and can therefore be created from the tuple (0, 0, ..., 0)
// by increasing each of its element until we reach the required offsets tuple.
// We start our iteration by adding the candidate (0, 0, ..., 0), and in each iteration we add into the candidates
// tree all permutations of the popped element generated by incrasing one of its offsets by 1 (as long as this candidate)
// wasn't already inserted into the candidates tree.
// Since there is a finite amount of records, and each iteration we pop a single element from the candidates tree, we can
// conclude every element that is inserted into the candidates tree will eventually be retrieved.
// Therefore, every tuple that can be generated from (0, 0, ... 0) by incrasing the elements in it (as long as we
// stay within the range of the tables' sizes) will eventually be added to the candidates tree, and therefore also retrieved.
// We can conclude that all elements will eventually be returned.
//
// Finally, we'll prove that elements are always returned in increasing provenance value.
// We'll prove that by induction, base case:
// Since the aggregation of provenance multiplication is guaranteed to be monotonous non-decreasing, and since the offets
// we're referring to correlate to offsets in tables' provenance indexes (which are sorted by ascending provenance score),
// we can rest assure the provenance score of the record in offsets (0, 0, ... 0) will be equal or less to the provenance
// scores of all other records in the joint table.
//
// induction step:
// Given that the first i records retrieved were the i records with the smallest provenance score in the joint records,
// we'll look at the i+1 record.
// It can't have a record smaller than any of the previous i records since they are the i smallest-provenance records and
// we never return the same record twice.
// We'll assume by negation that the i+1 record has a provenance score that is bigger than the i+1 biggest provenance score.
// This would mean that another record (which obviously was yet to be retrieved) has the i+1 score.
// Also, we would assume current record's provenance and the (i+1)th provenance are different since otherwise this case is null
// (the current record has the correct provenance score and this case doesn't contradict our assumptions).
// If this other record is in candidates, then it would've been retrieved in this iteration (since candidates always returns)
// the smallest provenance candidate within it.
// We can conclude the (i+1)th record isn't in candidates (and it wasn't returned) and therefore it will be added in the future
// to candidates by increasing the offsets of some candidate that will be popped out of candidates.
// That would mean it will have a provenance value that is not smaller than the candidate from which it was generated (since
// our aggregation function is monotonous non-decreasing), and therefore a score that is not smaller than the current smallest
// -score candidate.
// This contradicts our assumption that the current popped record has a score that is bigger than the (i+1)th biggest provenance
// score, and therefore the (i+1)th popped record must be the (i+1)th biggest provenance record.
//
